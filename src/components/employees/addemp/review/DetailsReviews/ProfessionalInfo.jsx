...
const ProfessionalInfo = ({profdata}) => {
  const [empdata, setEmpData] = useState({});
  const accessToken = getAccessTokenFromCookie();
  ...
  return (
    <div className="grid grid-cols-2 grid-rows-3 ">
  {profdata?.professional_information && Object.keys(profdata.professional_information).length > 0 && (
    <>
      <span className="mb-4">
        <h2 className="text-gray-400">Designation</h2>
        <p className="font-semibold text-base">{profdata?.professional_information?.emp_designation}</p>
      </span>
      <span>
        <h2 className="text-gray-400">Department</h2>
        <p className="font-semibold text-base">{profdata?.professional_information?.department_name}</p>
      </span>

      <span className="mb-4">
        <h2 className="text-gray-400">PF Number</h2>
        <p className="font-semibold text-base">{profdata?.professional_information?.pf}</p>
      </span>
      <span>
        <h2 className="text-gray-400">UAN Number</h2>
        <p className="font-semibold text-base">{profdata?.professional_information?.uan}</p>
      </span>

      <span className="mb-4">
        <h2 className="text-gray-400">Direct Reporting Manager</h2>
        <p className="font-semibold text-base">{`${profdata?.professional_information?.reporting_manager_first_name} ${profdata?.professional_information?.reporting_manager_last_name}`}</p>
      </span>
      <span>
        <h2 className="text-gray-400">Work location</h2>
        <p className="font-semibold text-base">{profdata?.professional_information?.work_location}</p>
      </span>
    </>
  )}
</div>
